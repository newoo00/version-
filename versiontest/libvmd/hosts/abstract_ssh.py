...
class AbstractSSHHost(base_classes.Host):
    """
    ...
    def _initialize(self, hostname, user="root", port=22, password="",
                    *args, **dargs):
        super(AbstractSSHHost, self)._initialize(*args, **dargs)
    ...
    def get_file(self, source, dest, delete_dest=False, preserve_perm=True,
        ...
        Raises:
                HostRunError: the scp command failed
        """
        ...
        if try_scp:
            ...
            if remote_source:
                ...
                try:
                    ...
                    raise error.HostRunError(e.args[0], e.args[1])
    ...
    def ssh_ping(self, timeout=60):
        ...
        except error.HostSSHTimeout:
            msg = "Host (ssh) verify timed out (timeout = %d)" % timeout
            raise error.HostSSHTimeout(msg)
        except error.HostSshPermissionDeniedError:
            #let HostSshPermissionDeniedError be visible to the callers
            raise
        except error.HostRunError, e:
            # convert the generic HostRunError into something more
            # specific for this context
            raise error.HostSshPingError(e.description + '\n' +
                                                 repr(e.result_obj))
    ...
    def verify_connectivity(self):
        ...
        if self.is_shutting_down():
            raise error.HostIsShuttingDownError("Host is shutting down")
...
#        os.remove(self.known_hosts_file)
    ...
    def start_master_ssh(self):
        ...
        if self.master_ssh_job is None:
            ...
            self.master_ssh_tempdir = temp.tempdir(unique_id='ssh-master')
