...
class SSHHost(abstract_ssh.AbstractSSHHost):
    ...
    def _run(self, command, timeout, ignore_status, stdout, stderr,
        ...
        if result.exit_status == 255:
            if re.search(r'^ssh: connect to host .* port .*: '
                ...
                raise error.HostSSHTimeout("ssh timed out", result)
            if "Permission denied." in result.stderr:
                msg = "ssh permission denied"
                raise error.HostSshPermissionDeniedError(msg, result)

        if not ignore_status and result.exit_status > 0:
            raise error.HostRunError("command execution error", result)
    ...
    def run(self, command, timeout=3600, ignore_status=False,
        ...
        try:
            ...
            raise error.HostRunError(cmderr.args[0], cmderr.args[1])
